...
def ServerUDP(PORT, MCASTPORT, MCASTGRP, SNDB, BSIZE, TTL, HOST=0, MULTI=False, IPV6=False):
    ...
    if IPV6 == False:
        ...
        regexmlt = re.search(
            '^((22[4-9]|23[0-9]).([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]).([0-9]|[1-9][0-9]|1[0-9][0-9]|2['
            '0-4][0-9]|25[0-5]).([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]))$',
            MCASTGRP)
        regex = re.search(
...
def Main():
    ...
    parser.add_argument('-ttl', '--timetolive', help='Select the ttl for your packets, default = 20', nargs='?',
                        default=20, type=int)
    parser.add_argument('-mp', '--mcastport', help='Port number of multicast, default port nr. is 8000', type=int,
                        default=8000, nargs='?')
    parser.add_argument('-mg', '--mcastgrp',
                        help='IP addr of multicast , default ipv4 => 224.0.0.1 ipv6 => ff15:7079:7468:6f6e:6465:6d6f:6d63:6173',
                        type=str, default='224.0.0.1', nargs='?')
    ...
    if args.server and args.TCP and not args.client and not args.UDP:
        ...
        ServerUDP(args.port, args.mcastport, args.mcastgrp, args.len, args.buffsize, args.timetolive, args.ip,
                  args.multicast, args.IPV6)
    elif args.client and args.UDP and not args.server and not args.TCP:
        ClientUDP(args.ip, args.port, args.mcastport, args.mcastgrp, args.len, args.buffsize, args.timetolive,
                  args.time, args.multicast, args.IPV6)
